...
# Note: This is a "one-time" script used to populate the initial data in the
# data/trusted_task_rules.yaml and data/trusted_task_rules_deprecated.yaml
# files It is not mean to be used again, so it could be removed from the repo,
# but I'm choosing to check it now in a reference showing how the data was
# created, and to potentially also be a starting point for some future
# automation for updating the trusted task data, e.g. when a new version of a
# task is released.
